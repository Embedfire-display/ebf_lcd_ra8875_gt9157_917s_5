...
BYTE bmpinfo[230402] __attribute__((at(0x680c0000)));
 ...
 * 图片宽度和高度根据图片大小而定
 */
void Lcd_show_bigbmp(unsigned short int x, unsigned short int y,unsigned char *pic_name)
{
	uint32_t j, k;
	uint16_t width, height, l_width;
	BYTE red,green,blue;
	BITMAPFILEHEADER bitHead;
	BITMAPINFOHEADER bitInfoHead;
	WORD fileType;
...
/*-------------------------------------------------------------------------------------------------------*/
	...
	if(bitInfoHead.biBitCount >= 24)
	{
		  //f_read(&bmpfsrc,bmpinfo,bitInfoHead.biSizeImage,&read_num);
			f_read(&bmpfsrc,bmpinfo,l_width*height,&read_num);
//			printf("bmpinfo:");
//			for(j=0;j<50;j++)
//				printf("%x",bmpinfo[j+100000]);
