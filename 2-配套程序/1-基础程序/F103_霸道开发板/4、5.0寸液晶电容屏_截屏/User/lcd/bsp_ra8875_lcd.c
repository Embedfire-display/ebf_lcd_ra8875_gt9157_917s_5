...
void RA8875_SetWriteDirection(uint8_t horizontal,uint16_t vertical)
{
 ...
 /**
  * @brief  RA8875_SetReadDirection设置液晶GRAM读取的方向(截屏bmp格式图片需要调整)
  * @param  
  *   @arg  horizontal|vertical   00 从左到右，然后上到下
                                  01 从右到左，然后上到下
                                  10 从上到下，然后左到右
	 ...
	 坐标系统示意图： （横屏）
      ...
      |         | 
			...
			|                    (800,480)|
			 -----------------------------
  */
void RA8875_SetReadDirection(uint8_t horizontal,uint16_t vertical)
{
  uint8_t temp;
  
  /*读取原始数据*/
  temp = RA8875_ReadReg(0x45);  
  
  /*设置水平方向*/
	RA8875_WriteReg(0x45, horizontal?(temp|(1<<0)):(temp&~(1<<0)));
     
  /*设置垂直方向*/
  temp = RA8875_ReadReg(0x45);

  RA8875_WriteReg(0x45, vertical?(temp|(1<<1)):(temp&~(1<<1)));
...
void LCD_Rst(void)
{			
		GPIO_ResetBits(GPIOG, GPIO_Pin_6);	 //低电平复位
    LCD_Delay(0xAFFf); 					   
