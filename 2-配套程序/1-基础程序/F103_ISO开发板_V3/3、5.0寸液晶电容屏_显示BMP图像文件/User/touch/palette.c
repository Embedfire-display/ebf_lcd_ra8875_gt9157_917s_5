...
static void LCD_DrawUniLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,uint8_t thick,uint16_t color );
  ...
  if(x>PALETTE_START_X && pre_x>PALETTE_START_X )
  {
    switch(brush->shape)  /*根据画刷参数描绘不同的轨迹*/
    {
      ...
      case LINE_SINGLE_PIXCEL:                 
            ...
            {      
              RA8875_PutPixel(x,y,brush->color);              
            }
            else //继续上一次的笔迹
            {      
              RA8875_DrawLine(pre_x,pre_y,x,y,brush->color);
            } 
      ...
      case LINE_2_PIXCEL:
        ...
        /* 描绘2像素宽度的轨迹线 */
        LCD_DrawUniLine(pre_x,pre_y,x,y,1,brush->color);
        
        break;
      
      case LINE_4_PIXCEL:
        
            LCD_DrawUniLine(pre_x,pre_y,x,y,2,brush->color);
      ...
      case LINE_6_PIXCEL:
        ...
        LCD_DrawUniLine(pre_x,pre_y,x,y,3,brush->color);
        
        break;
      
      case LINE_8_PIXCEL:
        
            LCD_DrawUniLine(pre_x,pre_y,x,y,4,brush->color);

        break;
      
     case LINE_16_PIXCEL:
        
            LCD_DrawUniLine(pre_x,pre_y,x,y,8,brush->color);

        break;
            
      case LINE_20_PIXCEL:
        
            LCD_DrawUniLine(pre_x,pre_y,x,y,10,brush->color);
      ...
      case LINE_WITH_CIRCLE:       
            ...
            {      
              RA8875_DrawLine(pre_x,pre_y,x,y,brush->color);
              RA8875_FillCircle(x,y,3,brush->color);
            } 
...
static void Draw_Clear_Button(void *btn)
{
  ...
  if(ptr->touch_flag == 0)
  {
    ...
    RA8875_SetBackColor(CL_BUTTON_GREY);
    RA8875_SetFont( RA_FONT_32, 0, 0);
    RA8875_DispStr(ptr->start_x + (ptr->end_x - ptr->start_x - 32*2 )/2,
                      ptr->start_y+25,
                      "清屏");
  }
  else  /*按键按下*/
 ...
 switch(ptr->para)
  {
    ...
    case LINE_SINGLE_PIXCEL:      
        ...
        RA8875_DrawLine(ptr->start_x+20,
    ...
    case LINE_4_PIXCEL:
     ...
     case LINE_WITH_CIRCLE: 
        ...
        RA8875_FillCircle(ptr->start_x+5+i*10,
                            ptr->start_y+(ptr->end_y-ptr->start_y)/2,
                            3,
                            CL_BLACK);
    ...
    case RUBBER:
       RA8875_FillRect( ptr->start_x+((ptr->end_x - ptr->start_x -40)/2),
      ...
      RA8875_DispStr(ptr->start_x+(ptr->end_x - ptr->start_x -24*2)/2,
                        ptr->end_y -30,
                        "橡皮");
...
static void Command_Clear_Palette(void *btn)
{
    RA8875_FillRect(PALETTE_START_X,
                      PALETTE_START_Y,
...
static void LCD_DrawUniLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,uint8_t thick,uint16_t color )
{
  ...
  {
    RA8875_FillCircle(x,y,thick,color);             /* Draw the current pixel */
