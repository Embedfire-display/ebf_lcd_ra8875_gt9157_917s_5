...
DRESULT TM_FATFS_FLASH_SPI_disk_ioctl(BYTE cmd, char *buff)
{
	switch (cmd) {
		...
		case GET_SECTOR_COUNT:
			*(DWORD * )buff = 1792;		//sector数量
			//*(DWORD * )buff = 1024;		//sector数量
		break;
		case CTRL_SYNC :
...
* Description    : 从外部Flash字库中读取自摸数据到指定的缓冲区 
...
{ 
    ...
    pos = ((High8bit-0xa0-16)*94+Low8bit-0xa0-1)*2*16;
		...
		SPI_FLASH_BufferRead(pBuffer,1900*4096+pos,32);			//读取数据
		return 0;  
//    myres=f_mount(0, &myfs);
//		//printf("\r\nGBK fmount myres=%d\r\n",myres);
//    myres = f_open(&myfsrc , "0:/HZLIB.bin", FA_OPEN_EXISTING | FA_READ);
//    if ( myres == FR_OK ) 
//    {
//        myres=f_lseek(&myfsrc, pos);		//指针偏移
//				//printf("\r\nGBK flseek myres=%d\r\n",myres);
